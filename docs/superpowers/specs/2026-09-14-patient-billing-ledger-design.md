...
- **Per-patient page:** *(revised after the spec was first written — `PatientProfileTabs.tsx`'s information/history/next/medical tab bar turned out to be dead code; a recent commit ("Remove single-patient-dashboard and restore PatientWorkspaceView") made `/admin/patients/[patientKey]` render the chairside charting workspace, `PatientWorkspaceView`, directly, with no tab bar. There's also a pre-existing, separate `/admin/patients/[patientKey]/workspace` route rendering the same component — untouched, not this feature's concern.)* Billing gets its own sibling route, `/admin/patients/[patientKey]/billing`, mirroring the existing `/workspace` route's page-composition pattern (`decodePatientKey` → `groupReservationsByPatient` → `getPatientGroup` → fetch). Reached via a small "Billing" link added to `WorkspaceHeader.tsx`'s floating name overlay (currently `pointer-events-none`; the link itself needs `pointer-events-auto`). New `PatientBillingView.tsx`: balance header, chronological entry list, "Record payment" / "Add charge" form.
