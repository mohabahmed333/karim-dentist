"use client";

import { Loader2Icon } from "lucide-react";
import { cmsTranslateSelectionLabel } from "../lib/collectCmsTranslateJobs";
import { useCmsTranslate } from "./CmsTranslateProvider";

export function CustomizeTranslateOverlay() {
  const { isTranslating, progress } = useCmsTranslate();
  if (!isTranslating || !progress) return null;

  const scopeLabel = cmsTranslateSelectionLabel(progress.selection);
  const label =
    progress.direction === "to-ar"
      ? `Translating ${scopeLabel} to Arabic…`
      : `Translating ${scopeLabel} to English…`;
  const percent =
    progress.total > 0
      ? Math.round((progress.done / progress.total) * 100)
      : 0;

  return (
    <div
      className="fixed inset-0 z-[600] flex items-center justify-center bg-[var(--admin-canvas)]/70 backdrop-blur-[2px]"
      role="alertdialog"
      aria-modal="true"
      aria-busy="true"
      aria-live="polite"
      aria-label={label}
    >
      <div className="mx-4 w-full max-w-sm rounded-[12px] border border-[var(--admin-border)] bg-[var(--admin-panel)] p-5 shadow-[0_16px_48px_rgba(0,0,0,0.16)]">
        <div className="flex items-center gap-3">
          <Loader2Icon
            className="size-5 shrink-0 animate-spin text-[var(--admin-text)]"
            aria-hidden
          />
          <div className="min-w-0">
            <p className="text-sm font-medium text-[var(--admin-text)]">{label}</p>
            <p className="mt-0.5 text-[12px] text-[var(--admin-muted)]">
              {progress.done} of {progress.total} fields · Please wait
            </p>
          </div>
        </div>
        <div className="mt-4 h-1.5 overflow-hidden rounded-full bg-[var(--admin-hover)]">
          <div
            className="h-full rounded-full bg-[var(--admin-text)] transition-[width] duration-200"
            style={{ width: `${percent}%` }}
          />
        </div>
      </div>
    </div>
  );
}
