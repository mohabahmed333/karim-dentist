...
import { AdminSkeleton as Skeleton } from "@/features/admin/components/AdminSkeleton";
...
const SKELETON_ROW_WIDTHS = ["w-40", "w-28", "w-36", "w-24", "w-32"];

function CommandPaletteResultsSkeleton() {
  return (
    <div aria-hidden>
      {SKELETON_ROW_WIDTHS.map((width, index) => (
        <div key={`${width}-${index}`} className="flex items-center gap-2.5 px-3 py-2">
      ...
      <div className="relative h-8 w-full">
        {!open ? (
          <motion.button
            ...
            className={cn(
              shellClass,
              "flex h-8 w-full items-center justify-start gap-2 rounded-full border border-[var(--admin-border)] px-2.5",
            )}
          >
              ...
              {open ? (
                <>
                  <motion.button
                    ...
                    className="fixed inset-0 z-(--z-drawer) bg-[#111111]/20"
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    ...
                    onClick={close}
                  />
                  <div className="pointer-events-none fixed inset-x-0 top-[4.25rem] z-(--z-drawer) flex justify-center px-3">
                    <motion.div
                      key="cmdk-shell"
                      layoutId={COMMAND_LAYOUT_ID}
                      role="dialog"
                      ...
                      className={cn(
                        shellClass,
                        "pointer-events-auto relative flex min-h-[20rem] w-[min(36rem,calc(100vw-1.5rem))] flex-col rounded-lg shadow-[0_18px_50px_rgba(15,23,42,0.14)]",
                      )}
                    >
                      ...
                      <div className="max-h-[min(26rem,58vh)] overflow-y-auto py-2">
