...
export function SupportAiControlPanel({ conversationId }: Props) {
  ...
  if (loading) {
    return (
      <p className="px-1 py-1 text-xs text-[var(--admin-muted)]">
  ...
  return (
    <div className="space-y-2.5 px-1 py-1">
      <label className="flex items-center justify-between gap-2 text-xs font-medium text-[var(--admin-text)]">
        ...
        <button
          ...
          className={cn(
            ...
            enabled ? "bg-[var(--admin-text)]" : "bg-[var(--admin-border)]",
          )}
        >
          <span
            className={cn(
              "absolute top-0.5 h-4 w-4 rounded-full bg-[var(--admin-panel)] transition-transform rtl:end-0.5",
      ...
      <div className="flex flex-wrap gap-1.5">
        {PAUSE_OPTIONS.map(({ minutes, key }) => (
          <button
            ...
            className="rounded-md border border-[var(--admin-border)] px-2 py-1 text-[11px] font-medium text-[var(--admin-text)] hover:bg-[var(--admin-hover)] disabled:opacity-50"
          >
            {t(key)}
          </button>
        ))}
        {isPaused ? (
          <button
            ...
            className="rounded-md border border-[var(--admin-border)] px-2 py-1 text-[11px] font-medium text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-500/15 disabled:opacity-50"
          >
