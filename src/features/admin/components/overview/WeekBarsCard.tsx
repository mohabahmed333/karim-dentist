...
export function WeekBarsCard({ weekCounts }: Props) {
  ...
  return (
    <section className="rounded-3xl bg-[var(--admin-panel)] p-6 shadow-sm ring-1 ring-[var(--admin-border)]">
      <h2 className="mb-4 text-sm font-semibold text-[var(--admin-text)]">This week</h2>
      <div className="flex items-end gap-2">
        {weekCounts.map((item) => (
          <div key={item.label} className="flex flex-1 flex-col items-center gap-2">
            ...
            <span className="text-xs font-medium text-[var(--admin-text)]">{item.count}</span>
          </div>
        ))}
      </div>
    </section>
